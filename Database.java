...
public class Database {
	private static final String URL = "jdbc:mysql://localhost:3306/cs320_project?useSSL=false&useUnicode=true&useLegacyDatetimeCode=false&serverTimezone=Turkey";
	...
	public static void getProducts() {
		try {
			...
			items items;
			while (rs.next()) {
				items = new items(rs.getInt("pid"), rs.getString("pname"), rs.getDouble("pprice"),
						rs.getString("pbrand"), rs.getInt("squantity"));
				products.add(items);
			}
	...
	public static void DBupdatePrice(int pid, double pprice) {
		try {
			...
			if (rs.next()) {
				...
				System.out.println("WARNING: The product's id was not found");
			}
			rs.close();
			statement.close();
			closeConnection();
